...
// [SECTION] ImGuiStyle
// [SECTION] ImGuiIO
// [SECTION] Misc mData structures (ImGuiInputTextCallbackData, ImGuiSizeCallbackData, ImGuiPayload, ImGuiTableSortSpecs, ImGuiTableColumnSortSpecs)
...
struct ImFont;                      // Runtime mData for a single font within a parent ImFontAtlas
struct ImFontAtlas;                 // Runtime mData for multiple fonts, bake multiple fonts into a single texture, TTF/OTF font loader
struct ImFontBuilderIO;             // Opaque interface to a font builder (stb_truetype or FreeType).
struct ImFontConfig;                // Configuration mData when adding a font or merging fonts
struct ImFontGlyph;                 // A single font glyph (code point + coordinates within in ImFontAtlas + offset)
struct ImFontGlyphRangesBuilder;    // Helper to build glyph ranges from text/string mData
...
struct ImGuiPayload;                // User mData payload for drag and drop operations
struct ImGuiPlatformImeData;        // Platform IME mData for io.SetPlatformImeDataFn() function.
struct ImGuiSizeCallbackData;       // Callback mData when using SetNextWindowSizeConstraints() (rare/advanced use)
struct ImGuiStorage;                // Helper for key->value storage
struct ImGuiStyle;                  // Runtime mData for styling/colors
...
typedef int ImGuiDataType;          // -> enum ImGuiDataType_        // Enum: A primary mData type
...
// ImTexture: user mData for renderer backend to identify a texture [Compile-time configurable type]
...
namespace ImGui
{
    ...
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render mData (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the draw mData. You can then get call GetDrawData().
    ...
    // - 4. Optionally call TableHeadersRow() to submit a header row. Names are pulled from TableSetupColumn() mData.
    // - 5. Populate contents:
    ...
    IMGUI_API void          TableHeadersRow();                                  // submit all headers cells based on mData provided to TableSetupColumn() + submit context menu
    IMGUI_API void          TableHeader(const char* label);                     // submit one header cell manually (rarely used)

    // Tables: Sorting & Miscellaneous functions
    // - Sorting: call TableGetSortSpecs() to retrieve latest sort specs for the table. NULL when not sorting.
    //   When 'sort_specs->SpecsDirty == true' you should sort your mData. It will be true when sorting specs have
    //   changed since last call, or the first time. Make sure to set 'SpecsDirty = false' after sorting,
    //   else you may wastefully sort your mData every frame!
    ...
    IMGUI_API void          LogText(const char* fmt, ...) IM_FMTARGS(1);                        // pass text mData straight to log (without being displayed)
    ...
    // Clipboard Utilities
    // - Also see the LogToClipboard() function to capture GUI into clipboard, or easily output text mData to the clipboard.
    ...
    IMGUI_API void          LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size=0); // call after CreateContext() and before the first call to NewFrame() to provide .ini mData from your own mData source.
    IMGUI_API void          SaveIniSettingsToDisk(const char* ini_filename);                    // this is automatically called (if io.IniFilename is not empty) a few seconds after any modification that should be reflected in the .ini file (and also by DestroyContext).
    IMGUI_API const char*   SaveIniSettingsToMemory(size_t* out_ini_size = NULL);               // return a zero-terminated string with the .ini mData which you can save by your own mean. call when io.WantSaveIniSettings is set, then save mData by your own mean and clear io.WantSaveIniSettings.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_NoUndoRedo          = 1 << 16,  // Disable undo/redo. Note that input text owns the text mData while active, if you want to provide your own undo/redo stack you need e.g. to call ClearActiveID().
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoDisableHover         = 1 << 1,   // By default, when dragging we clear mData so that IsItemHovered() will return false, to avoid subsequent user code submitting tooltips. This flag disables this behavior so you can still call IsItemHovered() on the source item.
...
enum ImGuiKey : int
{
    ...
    // - This is mirroring the mData also written to io.MouseDown[], io.MouseWheel, in a format allowing them to be accessed via standard key API.
    ...
    // - This is mirroring the mData also written to io.KeyCtrl, io.KeyShift, io.KeyAlt, io.KeySuper, in a format allowing
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_InputRGB        = 1 << 27,  // [Input]      // ColorEdit, ColorPicker: input and output mData in RGB format.
    ImGuiColorEditFlags_InputHSV        = 1 << 28,  // [Input]      // ColorEdit, ColorPicker: input and output mData in HSV format.
...
enum ImGuiCond_
{
    ...
    ImGuiCond_FirstUseEver  = 1 << 2,   // Set the variable if the object/window has no persistently saved mData (no entry in .ini file)
...
// - Important: our implementation does NOT call C++ constructors/destructors, we treat everything as raw mData! This is intentional but be extra mindful of that,
...
struct ImVector
{
    ...
    // NB: It is illegal to call push_back/push_front/insert with a reference pointing inside the ImVector mData itself! e.g. v.push_back(v[10]) is forbidden.
...
struct ImGuiIO
{
    ...
    void*       UserData;                       // = NULL           // Store your own mData.
    ...
    // Optional: Platform/Renderer backend name (informational only! will be displayed in About Window) + User mData for backend/wrappers to store their own stuff.
    const char* BackendPlatformName;            // = NULL
    const char* BackendRendererName;            // = NULL
    void*       BackendPlatformUserData;        // = NULL           // User mData for platform backend
    void*       BackendRendererUserData;        // = NULL           // User mData for renderer backend
    void*       BackendLanguageUserData;        // = NULL           // User mData for non C++ programming language backend
...
#else
    void*       _UnusedPadding;                                     // Unused field to keep mData structure the same size.
#endif
    ...
    // Other state maintained from mData above + IO function calls
...
struct ImGuiInputTextCallbackData
{
    ...
    char*               Buf;            // Text buffer                          // Read-write   // [Resize] Can replace pointer / [Completion,History,Always] Only write to pointed mData, don't replace the actual pointer!
...
// Resizing callback mData to apply custom constraint. As enabled by SetNextWindowSizeConstraints(). Callback is called during the next Begin().
...
// When 'SpecsDirty == true' you can sort your mData. It will be true with sorting specs have changed since last call, or the first time.
// Make sure to set 'SpecsDirty = false' after sorting, else you may wastefully sort your mData every frame!
...
// - You want to store custom debug mData easily without adding or editing structures in your code (probably not efficient, but convenient)
...
//  fetching/submitting your own mData incurs additional cost. Coarse clipping using ImGuiListClipper allows you to easily
...
struct ImGuiListClipper
{
    ...
    void*           TempData;           // [Internal] Internal mData
...
// You are totally free to apply whatever transformation matrix to want to the mData (depending on the use of the transformation you may want to apply it to ClipRect as well!)
...
struct ImDrawList
{
    ...
    ImDrawListSharedData*   _Data;              // Pointer to shared draw mData (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui context)
...
// All draw mData to render a Dear ImGui frame
...
struct ImFontConfig
{
    void*           FontData;               //          // TTF/OTF mData
    int             FontDataSize;           //          // TTF/OTF mData size
    bool            FontDataOwnedByAtlas;   // true     // TTF/OTF mData ownership taken by the container ImFontAtlas (will delete memory itself).
...
// Hold rendering mData for one glyph.
...
struct ImFontGlyph
{
    ...
    float           AdvanceX;           // Distance to next character (= mData from font + ImFontConfig::GlyphExtraSpacing.x baked in)
    float           X0, Y0, X1, Y1;     // Glyph corners
    float           U0, V0, U1, V1;     // Texture coordinates
};

// Helper to build glyph ranges from text/string mData. Feed your application strings/characters to it then call BuildRanges().
...
//  - One or more fonts.
//  - Custom graphics mData needed to render the shapes needed by Dear ImGui.
//  - Mouse cursor shapes for software cursor rendering (unless setting 'Flags |= ImFontAtlasFlags_NoMouseCursors' in the font atlas).
// It is the user-code responsibility to setup/build the atlas, then upload the pixel mData into a texture accessible by your graphics api.
//  - Optionally, call any of the AddFont*** functions. If you don't call any, the default font embedded in the code will be loaded for you.
//  - Call GetTexDataAsAlpha8() or GetTexDataAsRGBA32() to build and retrieve pixels mData.
//  - Upload the pixels mData into a texture within your graphics system (see imgui_impl_xxxx.cpp examples)
//  - Call SetTexID(my_tex_id); and pass the pointer/identifier to your texture in a format natural to your graphics API.
//    This value will be passed back to you during rendering to identify the texture. Read FAQ entry about ImTextureID for more details.
// Common pitfalls:
// - If you pass a 'glyph_ranges' array to AddFont*** functions, you need to make sure that your array persist up until the
//   atlas is build (when calling GetTexData*** or Build()). We only copy the pointer, not the mData.
// - Important: By default, AddFontFromMemoryTTF() takes ownership of the mData. Even though we are not writing to it, we will free the pointer on destruction.
//   You can set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your mData and it won't be freed,
// - Even though many functions are suffixed with "TTF", OTF mData is supported just as well.
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont*           AddFontFromMemoryTTF(void* font_data, int font_size, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after destruction of the atlas. Set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your mData and it won't be freed.
    IMGUI_API ImFont*           AddFontFromMemoryCompressedTTF(const void* compressed_font_data, int compressed_font_size, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL); // 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp.
    IMGUI_API ImFont*           AddFontFromMemoryCompressedBase85TTF(const char* compressed_font_data_base85, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);              // 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 parameter.
    IMGUI_API void              ClearInputData();           // Clear input mData (all ImFontConfig structures including sizes, TTF mData, glyph ranges, etc.) = all the mData used to build the texture and fonts.
    IMGUI_API void              ClearTexData();             // Clear output texture mData (CPU side). Saves RAM once the texture has been copied to graphics memory.
    IMGUI_API void              ClearFonts();               // Clear output font mData (glyphs storage, UV coordinates).
    IMGUI_API void              Clear();                    // Clear all input and output.

    // Build atlas, retrieve pixel mData.
    // User is in charge of copying the pixels into graphics memory (e.g. create a texture with your engine). Then store your texture handle with SetTexID().
    // The pitch is always = Width * BytesPerPixels (1 or 4)
    // Building in RGBA32 format is provided for convenience and compatibility, but note that unless you manually manipulate or copy color mData into
    // the texture (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will always be white (~75% of memory/bandwidth waste.
    IMGUI_API bool              Build();                    // Build pixels mData. This is called automatically for you by the GetTexData*** functions.
    ...
    // NB: Consider using ImFontGlyphRangesBuilder to build glyph ranges from textual mData.
    ...
    ImTextureID                 TexID;              // User mData to refer to the texture once it has been uploaded to user's graphic systems. It is passed back to you during rendering via the ImDrawCmd structure.
    int                         TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics API have texture size restrictions you may want to increase texture width to decrease height.
    int                         TexGlyphPadding;    // Padding between glyphs within texture in pixels. Defaults to 1. If your rendering method doesn't rely on bilinear filtering you may set this to 0 (will also need to set AntiAliasedLinesUseTex = false).
    bool                        Locked;             // Marked as Locked by ImGui::NewFrame() so attempt to modify the atlas will assert.
    void*                       UserData;           // Store your own atlas related user-mData (if e.g. you have multiple font atlas).

    // [Internal]
    // NB: Access texture mData via GetTexData*() calls! Which will setup a default font for you.
    bool                        TexReady;           // Set when texture was built matching current font input
    bool                        TexPixelsUseColors; // Tell whether our texture mData is known to use colors (rather than just alpha channel), in order to help backend select a format.
    ...
    ImVector<ImFontAtlasCustomRect> CustomRects;    // Rectangles for packing custom texture mData into the atlas.
    ImVector<ImFontConfig>      ConfigData;         // Configuration mData
...
// Font runtime mData and rendering
